        ...
        if torch.cuda.is_available():
            ...
            if (
                ...
                for config_file in ["32k.json", "40k.json", "48k.json"]:
                    with open(f"../libs/rvc/configs/{config_file}", "r") as f:
                        strr = f.read().replace("true", "false")
                    with open(f"configs/{config_file}", "w") as f:
                        f.write(strr)
                with open("../libs/rvc/trainset_preprocess_pipeline_print.py", "r") as f:
                    strr = f.read().replace("3.7", "3.0")
                with open("../libs/rvc/trainset_preprocess_pipeline_print.py", "w") as f:
                    f.write(strr)
            else:
                self.gpu_name = None
            self.gpu_mem = int(
